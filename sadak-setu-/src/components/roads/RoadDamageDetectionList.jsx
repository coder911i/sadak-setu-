          ...
          <div className="flex items-center gap-1.5 p-1 bg-slate-950/80 rounded-xl border border-slate-800 text-xs">
            <button
              ...
              className={`px-3 py-1 rounded-lg font-semibold transition-all cursor-pointer ${
                filterType === 'all' ? 'bg-brand-600 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              All ({defects.length})
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-lg font-semibold transition-all cursor-pointer ${
                filterType === 'pothole' ? 'bg-rose-600 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg font-semibold transition-all cursor-pointer ${
                filterType === 'crack' ? 'bg-orange-600 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Cracks ({cracksCount})
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-lg font-semibold transition-all cursor-pointer ${
                filterType === 'other' ? 'bg-amber-600 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Other ({otherCount})
            </button>
          ...
          {filtered.map((item) => {
            ...
            return (
              <div
                ...
                className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 hover:border-slate-700 transition-all cursor-pointer space-y-3 group shadow-sm flex flex-col justify-between"
              >
                {/* Header info */}
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <div className="text-[11px] font-mono text-brand-400 mt-0.5">
                  ...
                  <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-emerald-950 text-emerald-300 border border-emerald-800 flex items-center gap-1">
                    <Cpu className="w-3 h-3 text-emerald-400" /> {item.confidenceScore}% Conf
                  </span>
                </div>

                {/* Simulated Visual Detection Frame */}
                <div className="relative aspect-[16/8] rounded-lg bg-slate-900 border border-slate-800 overflow-hidden flex items-center justify-center">
                  ...
                  <div className="absolute bottom-2 left-2 px-2 py-0.5 rounded bg-slate-950/80 border border-slate-750 text-[9px] font-mono text-slate-300">
                    GPS: {item.lat?.toFixed(4)}° N, {item.lng?.toFixed(4)}° E
                  </div>

                  <div className="absolute top-2 right-2 px-1.5 py-0.2 rounded bg-slate-950/80 font-mono text-[9px] text-slate-400">
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-400 pt-1 border-t border-slate-850">
                  ...
                  <span className="text-brand-400 group-hover:underline flex items-center gap-1 font-semibold">
